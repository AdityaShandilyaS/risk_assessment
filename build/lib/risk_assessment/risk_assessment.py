...
class RiskAssessment:
    def __init__(self):
        self.fs = file_ops.FileSystem()

    # filename must include .csv but model must NOT include .pkl
    def predict_risk(self, filename=None, modelname=None):
        file_path = os.path.join(self.fs.main_dir, filename)
        if os.path.exists(file_path):
            self.fs.copy_file(filename, filetype='prediction')
        ml = data_ops.MLModule(modelname)
        ml.load_model(modelname)
        ml.predict(filename)

    def train_model(self, filename=None, modelname=None, splitter=None, max_depth=None, min_samples_split=None):
        if filename:
            file_path = os.path.join(self.fs.main_dir, filename)
            if os.path.exists(file_path):
                self.fs.copy_file(filename, filetype='training')
        ml = data_ops.MLModule(modelname)
        ml.preprocess_data()
        ml.analyse_dataset()
